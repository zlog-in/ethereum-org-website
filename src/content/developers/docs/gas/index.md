...
The total fee would now be: `units of gas used * (base fee per gas + priority fee per gas)` where the `base fee` is a value set by the protocol and the `priority fee` is a value set by the user as a tip to the validator.
